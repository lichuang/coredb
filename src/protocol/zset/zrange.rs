...
impl Command for ZRangeCommand {
  async fn execute(&self, items: &[Value], server: &Server) -> Result<Value, CoreDbError> {
    ...
    // Sort by score ascending, then by member name lexicographically for equal scores
    members.sort_by(|a, b| {
      a.1
        .partial_cmp(&b.1)
...
mod tests {
  ...
  fn test_parse_args_basic() {
    let items = vec![ss("ZRANGE"), bulk(b"myzset"), ss("0"), bulk(b"-1")];
    let args = ZRangeCommand::parse_args(&items).unwrap();
  ...
  fn test_parse_args_positive_indices() {
    let items = vec![ss("ZRANGE"), bulk(b"myzset"), ss("1"), bulk(b"3")];
    let args = ZRangeCommand::parse_args(&items).unwrap();
  ...
  fn test_parse_args_negative_indices() {
    let items = vec![ss("ZRANGE"), bulk(b"myzset"), ss("-3"), bulk(b"-1")];
    let args = ZRangeCommand::parse_args(&items).unwrap();
  ...
  fn test_parse_args_insufficient() {
    let items = vec![ss("ZRANGE"), bulk(b"myzset")];
    let result = ZRangeCommand::parse_args(&items);
    assert!(result.is_err());
  }

  #[test]
  fn test_parse_args_no_args() {
    let items = vec![ss("ZRANGE")];
    let result = ZRangeCommand::parse_args(&items);
    assert!(result.is_err());
  }
  ...
  fn test_parse_args_invalid_start() {
    let items = vec![ss("ZRANGE"), bulk(b"myzset"), ss("abc"), bulk(b"1")];
    let result = ZRangeCommand::parse_args(&items);
    assert!(result.is_err());
  }
  ...
  fn test_parse_args_invalid_stop() {
    let items = vec![ss("ZRANGE"), bulk(b"myzset"), ss("0"), bulk(b"abc")];
    let result = ZRangeCommand::parse_args(&items);
    assert!(result.is_err());
  }
  ...
  fn test_parse_args_invalid_key_type() {
    let items = vec![ss("ZRANGE"), Value::Integer(42), ss("0"), bulk(b"-1")];
    let result = ZRangeCommand::parse_args(&items);
    assert!(result.is_err());
  }
